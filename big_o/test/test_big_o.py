...
class TestBigO(unittest.TestCase):
    def test_measure_execution_time(self):
        ...
        assert_array_almost_equal(t * 10.0, np.arange(1, 6), 1)

    def test_infer_big_o(self):
        x = np.linspace(10, 100, 100)

        desired = [
            (lambda x: x * 0.0 + 2.0, compl.Constant, [2.0]),
            (lambda x: 4.0 * x, compl.Linear, [0.0, 4.0]),
            (lambda x: 3.0 * x ** 2.0, compl.Quadratic, [0.0, 3.0]),
            (lambda x: 2.5 * x ** 3 + 2.0, compl.Cubic, [2.0, 2.5]),
            (lambda x: 2.0 * x ** 4.0, compl.Polynomial, [np.log(2.0), 4.0]),
            (lambda x: 1.5 * np.log(x), compl.Logarithmic, [0.0, 1.5]),
            (lambda x: x * np.log(x), compl.Linearithmic, [0.0, 1.0]),
            (lambda x: 0.6 ** x, compl.Exponential, [0.0, np.log(0.6)]),
        ]
